<template>
    <view class="image-cropper-wrapper">
        <view class="bg_container">
            <view class="bg_top" style="height: 100px;"></view>
		...
		<div class="loading" :style="{visibility: loading}">
			<IconFont name="loading1" size="25vw" color="#ffffff"></IconFont>
		</div>

        <canvas type="2d" id="my-canvas" disableScroll="true" class="my-canvas-class" :style="{ top: _canvas_top + 'px' }" ></canvas>
    </view>
</template>
<script>
...
export default {
    ...
    setup() {
		...
		const _draw = () => {
			let img_width = state._img_width * state.scale;
			let img_height = state._img_height * state.scale;
			let distX = state._img_left - (state._img_width * (state.scale - 1)) / 2 - state._cut_left;
			let distY = state._img_top - (state._img_height * (state.scale - 1)) / 2 - state._cut_top;

			Taro.createSelectorQuery().select('#my-canvas').fields({node: true, size: true}).exec((res) => {
				const canvas = res[0].node;
				canvas.width = state._cut_width;
				canvas.height = state._cut_height;
				const ctx = canvas.getContext('2d');
				ctx.translate(distX, distY);
				ctx.clearRect(0, 0, state._cut_width, state._cut_height);
					...
					Taro.canvasToTempFilePath({
						...
						success: function(res) {
							...
							Taro.uploadFile({
								...
								success: function (res) {
									Taro.eventCenter.trigger('ocrResult', res.data);
						...
						fail: function(res) {
							Taro.eventCenter.trigger('ocrResult', 'Canvas画布导出图片失败，请重试!');
							state.loading = 'hidden';
							state.img = '';
							Taro.eventCenter.trigger('cancelOcr', null);
						}
